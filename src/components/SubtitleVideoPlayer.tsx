...
import { captureVideoFrameToBlob, saveThumbnailBlob } from "@/lib/thumbnailUtils";
...
const SubtitleVideoPlayer = ({
  ...
  const handleDownload = () => {
    ...
    const baseUrl = currentServer === "backup" && backupVideoUrl ? backupVideoUrl : videoUrl;
    const url = currentQuality !== "1080p" ? getCurrentVideoUrl() : baseUrl;
    const label = currentQuality;
    if (!url) {
  ...
  useEffect(() => {
    ...
    const handleTimeUpdate = () => {
      ...
      // Auto-capture thumbnail from canvas when playing reaches a good scene
      const thumbnailTargetTime = opEndSec ? opEndSec + 30 : 90;
      if (
        !capturedThumbnailRef.current &&
        !thumbnailUrl &&
        animeId &&
        episodeId &&
        time >= thumbnailTargetTime
      ) {
        capturedThumbnailRef.current = true;
        captureVideoFrameToBlob(video).then((blob) => {
          if (blob) {
            saveThumbnailBlob(blob, animeId, episodeId);
          }
        });
      }
    };
  ...
  }, [opStartSec, opEndSec, edStartSec, edEndSec, hasNextEpisode, onNextEpisode, saveCurrentProgress, thumbnailUrl, animeId, episodeId]);
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2 md:gap-4">
                ...
                <motion.button
                  ...
                  title={`Letöltés a kiválasztott minőségben – napi limit: ${DOWNLOAD_DAILY_LIMIT}/nap`}
                >
