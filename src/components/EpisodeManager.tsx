...
import { Plus, Trash2, Play, Loader2, X, ChevronDown, ChevronUp, Pencil, Save, Upload, FileText, Camera } from "lucide-react";
...
import { autoGenerateThumbnailFromUrl } from "@/lib/thumbnailUtils";
...
const EpisodeManager = ({ animeId, animeTitle, onClose }: EpisodeManagerProps) => {
  ...
  const [thumbnailEpisode, setThumbnailEpisode] = useState<Episode | null>(null);
  ...
  const addEpisodeMutation = useMutation({
    mutationFn: async (episode: EpisodeFormData) => {
      ...
      if (error) throw error;
      return { episodeNumber: episode.episode_number, episodeId: data.id, videoUrl: episode.video_url.trim(), opEnd: episode.op_end.trim() };
    },
    onSuccess: ({ episodeNumber, episodeId, videoUrl, opEnd }) => {
      queryClient.invalidateQueries({ queryKey: ["episodes", animeId] });
      resetAddForm();
      toast.success("Epizód hozzáadva! Thumbnail generálás folyamatban...");

      // Auto-generate thumbnail in background
      const opEndSecs = opEnd ? (() => {
        const parts = opEnd.split(":").map(Number);
        if (parts.length === 2) return parts[0] * 60 + parts[1];
        if (parts.length === 3) return parts[0] * 3600 + parts[1] * 60 + parts[2];
        return null;
      })() : null;
      const timestamp = opEndSecs ? opEndSecs + 30 : 90;
      autoGenerateThumbnailFromUrl(animeId, episodeId, videoUrl, timestamp).then(() => {
        queryClient.invalidateQueries({ queryKey: ["episodes", animeId] });
      });
  ...
  const updateEpisodeMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: EpisodeFormData }) => {
      const { error } = await supabase
        ...
        .eq("id", id);
      if (error) throw error;
      return { id, videoUrl: data.video_url.trim(), opEnd: data.op_end.trim() };
    },
    onSuccess: ({ id, videoUrl, opEnd }) => {
      queryClient.invalidateQueries({ queryKey: ["episodes", animeId] });
      cancelEdit();
      toast.success("Epizód frissítve! Thumbnail újragenerálás folyamatban...");

      // Re-generate thumbnail in background
      const opEndSecs = opEnd ? (() => {
        const parts = opEnd.split(":").map(Number);
        if (parts.length === 2) return parts[0] * 60 + parts[1];
        if (parts.length === 3) return parts[0] * 3600 + parts[1] * 60 + parts[2];
      ...
      const timestamp = opEndSecs ? opEndSecs + 30 : 90;
      autoGenerateThumbnailFromUrl(animeId, id, videoUrl, timestamp).then(() => {
        queryClient.invalidateQueries({ queryKey: ["episodes", animeId] });
      });
    },
    onError: (error: Error) => {
  ...
  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        <Button
                          variant="ghost"
                          size="icon"
                          onClick={() => setThumbnailEpisode(episode)}
                          className={episode.thumbnail_url ? "text-green-400 hover:text-green-300" : "text-muted-foreground hover:text-primary"}
                          title="Thumbnail generálás"
    ...
    {thumbnailEpisode && (
      <ThumbnailCapture
        episodeId={thumbnailEpisode.id}
        animeId={animeId}
        videoUrl={thumbnailEpisode.video_url}
        currentThumbnail={thumbnailEpisode.thumbnail_url}
        onClose={() => setThumbnailEpisode(null)}
        onSaved={() => {
          queryClient.invalidateQueries({ queryKey: ["episodes", animeId] });
          setThumbnailEpisode(null);
        }}
      />
    )}
    </>
