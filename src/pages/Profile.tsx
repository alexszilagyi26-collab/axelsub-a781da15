...
import { useProfile, useUpdateProfile, useWatchStats, useUploadAvatar, useUploadBackground } from "@/hooks/useProfile";
...
const COLOR_PRESETS = [
  { label: "Lila (alapértelmezett)", value: "#8B5CF6" },
  { label: "Kék", value: "#3B82F6" },
  { label: "Piros", value: "#EF4444" },
  { label: "Zöld", value: "#10B981" },
  { label: "Narancs", value: "#F97316" },
  { label: "Rózsaszín", value: "#EC4899" },
  { label: "Sárga", value: "#EAB308" },
  { label: "Türkiz", value: "#06B6D4" },
];
...
const Profile = () => {
  ...
  const handleBackgroundChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast({ title: "Háttérkép frissítve" });
    } catch (error) {
      toast({ title: "Hiba történt", variant: "destructive" });
    }
  };
  ...
  const handleColorSelect = async (color: string) => {
    ...
    try {
      await updateProfile.mutateAsync({ accent_color: color });
      toast({ title: "Szín frissítve" });
    } catch (error) {
      toast({ title: "Hiba történt", variant: "destructive" });
    }
  };
  ...
  const handleRemoveBackground = async () => {
    try {
      ...
      toast({ title: "Háttérkép eltávolítva" });
    } catch (error) {
      toast({ title: "Hiba történt", variant: "destructive" });
    }
  };
  ...
  const currentColor = selectedColor || profile?.accent_color || "#8B5CF6";
      ...
      <main className="pt-16 pb-24 md:pb-8">
        <div className="container mx-auto px-4 py-8">
          ...
          >
            ...
            {/* Color Picker */}
            <div className="mb-6">
              <p className="text-sm font-medium text-muted-foreground mb-3">Kiemelő szín</p>
              <div className="flex flex-wrap gap-3 items-center">
                {COLOR_PRESETS.map((preset) => (
                  <button
                    ...
                    className="w-9 h-9 rounded-full border-2 transition-all hover:scale-110 focus:outline-none"
                    style={{
                      backgroundColor: preset.value,
                      borderColor: currentColor === preset.value ? "white" : "transparent",
                      boxShadow: currentColor === preset.value ? `0 0 0 2px ${preset.value}` : "none",
                    }}
                  />
                ))}
                {/* Custom color input */}
                <div className="relative">
                  <input
                    ...
                    className="w-9 h-9 rounded-full cursor-pointer border-2 border-border bg-transparent"
                    title="Egyéni szín"
                  />
                </div>
                <span className="text-xs text-muted-foreground ml-1">Egyéni</span>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                >
                  ...
                  {uploadBackground.isPending ? "Feltöltés..." : profile?.background_url ? "Csere" : "Feltöltés"}
                </Button>
                {profile?.background_url && (
                  <Button
                    variant="outline"
                ...
                <p className="text-xs text-muted-foreground w-full mt-1">
                  A háttérkép az egész oldal mögött jelenik meg
                </p>
              </div>
            </div>
          </motion.div>
