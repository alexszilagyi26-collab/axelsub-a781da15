...
async function createServer() {
  ...
  app.use((req, res, next) => {
    ...
    const isReplitDev = REPLIT_DEV_DOMAIN && origin && origin.includes(REPLIT_DEV_DOMAIN);
    if (origin && (ALLOWED_ORIGINS.includes(origin) || !isProd || isReplitDev)) {
...
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#13132a;border-radius:16px;overflow:hidden;border:1px solid #2a2a4a;">
  ...
  // --- Extract a video frame as JPEG thumbnail using ffmpeg ---
  app.post("/api/extract-frame", async (req, res) => {
    const { videoUrl, timestamp = 30 } = req.body || {};
    if (!videoUrl) return res.status(400).json({ ok: false, error: "videoUrl kötelező" });

    try {
      const { spawn } = await import("child_process");
      const args = [
        "-y",
        "-ss", String(Number(timestamp) || 30),
        "-i", videoUrl,
        "-vframes", "1",
        "-vf", "scale=640:-2",
        "-f", "image2pipe",
      ...
      await new Promise((resolve, reject) => {
        proc.on("close", () => {
          ...
          if (chunks.length === 0) {
            reject(new Error("ffmpeg nem tudott képkockát kinyerni. Próbálj kisebb időbélyeget (pl. 30)."));
          } else {
            resolve(null);
          }
        });
      ...
      res.json({ ok: true, image: `data:image/jpeg;base64,${base64}` });
    } catch (err) {
      console.error("[extract-frame] hiba:", err.message);
      res.status(500).json({ ok: false, error: err.message });
    }
  });
